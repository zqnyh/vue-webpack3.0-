...
module.exports = function (content, filename, needMap) {
  ...
  fragment.childNodes.forEach(function (node) {
    ...
    if ((type === 'script' || type === 'template') && output[type].length > 0) {
      throw new Error('[vdoc-loader] Only one <script> or <template> tag is ' + 'allowed inside a Vue component.');
    }
    ...
    if (type === 'script') {
      ...
      result = commentScript(content.slice(0, start), lang) + deindent(content.slice(start, end)) + commentScript(content.slice(end), lang);
    } else {
...
function commentScript(content, lang) {
  ...
  return content.split(splitRE).map(function (line) {
